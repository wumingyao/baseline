...
parser.add_argument('--n_route', type=int, default=560)
...
TaiAn = data_gen(pjoin('./dataset', data_file), (n_train, n_val, n_test), n, n_his + n_pred)
print(f'>> Loading dataset with Mean: {TaiAn.mean:.2f}, STD: {TaiAn.std:.2f}')

if __name__ == '__main__':
    # model_train(TaiAn, blocks, args,path='./output60min/')
    model_test(TaiAn, TaiAn.get_len('test'), n_his, n_pred, args.inf_mode,path='./output60min/')
